...
func TestListSubscriptionsNoOptions(t *testing.T) {
	...
	pushpad.Configure("TOKEN", 0)
	subscriptions, total, err := List(&SubscriptionListParams{ProjectID: 123})
	if err != nil {
		t.Fatalf("expected no error, got %s", err)
	}
	if total != 0 {
...
func TestCreateSubscription(t *testing.T) {
	...
	pushpad.Configure("TOKEN", 0)
	subscription, err := Create(&SubscriptionCreateParams{ProjectID: 123, Endpoint: "https://example.com/1"})
	if err != nil {
		t.Fatalf("expected no error, got %s", err)
	}
...
func TestGetSubscription(t *testing.T) {
	...
	pushpad.Configure("TOKEN", 0)
	subscription, err := Get(50, &SubscriptionGetParams{ProjectID: 123})
	if err != nil {
		t.Fatalf("expected no error, got %s", err)
	}
	if subscription.ID != 50 {
...
func TestUpdateSubscription(t *testing.T) {
	...
	pushpad.Configure("TOKEN", 0)
	uid := "new-user"
	update := &SubscriptionUpdateParams{ProjectID: 123, UID: &uid}
...
func TestDeleteSubscription(t *testing.T) {
	...
	pushpad.Configure("TOKEN", 0)
	if err := Delete(50, &SubscriptionDeleteParams{ProjectID: 123}); err != nil {
		t.Fatalf("expected no error, got %s", err)
	}
}
